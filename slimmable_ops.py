...
width_mult = FLAGS.width_mult_range[-1]  # 从这里调用 FLAGS
...
# * 构建共享参数的 conv 层，因为有不同的 channel，所以会取最大的 channel，构建公共的
class SlimmableConv2d(nn.Conv2d):  # Slimmable
    def __init__(
        self,
        in_channels_list,
        out_channels_list,
        ...
        super(SlimmableConv2d, self).__init__(  # 调用父函数的 init，
            int(max(in_channels_list)),
            int(max(out_channels_list)),
            kernel_size,
            stride=stride,
            padding=padding,
            dilation=dilation,
        ...
        if self.groups_list == [1]:
            self.groups_list = [1 for _ in range(len(in_channels_list))]  # 输出的 list
    ...
    def forward(self, input):
        ...
        weight = self.weight[: self.out_channels, : self.in_channels, :, :]  # 取出 weight，。 x @ W^T #** 取出权重的切片
        if self.bias is not None:
            bias = self.bias[: self.out_channels]  # ** 取出bias 的权重
        else:
            bias = self.bias
        y = nn.functional.conv2d(input, weight, bias, self.stride, self.padding, self.dilation, self.groups)  # * 传入conv
        return y
...
class USConv2d(nn.Conv2d):
    def __init__(
        ...
        if us[0]:  # * 控制输入和输出
            in_channels_max = int(make_divisible(in_channels * width_mult / ratio[0]) * ratio[0])
        if us[1]:
            out_channels_max = int(make_divisible(out_channels * width_mult / ratio[1]) * ratio[1])
    ...
    def forward(self, input):
        ...
        if self.us[0]:
            in_channels = int(make_divisible(self.in_channels_basic * self.width_mult / self.ratio[0]) * self.ratio[0])
        if self.us[1]:
            out_channels = int(make_divisible(self.out_channels_basic * self.width_mult / self.ratio[1]) * self.ratio[1])
...
class SlimmableLinear(nn.Linear):
    def __init__(self, in_features_list, out_features_list, bias=True):
        super(SlimmableLinear, self).__init__(max(in_features_list), max(out_features_list), bias=bias)  # 初始化，取最大的尺度进行
    ...
    def forward(self, input):
        ...
        weight = self.weight[: self.out_features, : self.in_features]  # * 参数共享
        if self.bias is not None:
...
class USLinear(nn.Linear):
    def __init__(self, in_features, out_features, bias=True, us=[True, True]):
        ...
        if us[0]:
            in_features_max = make_divisible(in_features * width_mult)
        if us[1]:
            out_features_max = make_divisible(out_features * width_mult)
        super(USLinear, self).__init__(in_features_max, out_features_max, bias=bias)
...
# done 对每个[resolution, depth, width]分别做
class SwitchableBatchNorm2d(nn.Module):  # * 可切换的 BN
    def __init__(self, num_features_list):
        super(SwitchableBatchNorm2d, self).__init__()
        self.num_features_list = num_features_list  # 得到 feature list
        self.num_features = max(num_features_list)

        bns = []  # ** 所以，对 [resolution, depth, width]进行组合，共计 4*2*2=16个 BN
        for r in range(len(FLAGS.resolution_list)):
            for d in range(len(FLAGS.depth_mult_range)):
                for i in num_features_list:
                    bns.append(nn.BatchNorm2d(i))
        self.bn = nn.ModuleList(bns)  # 转成 ModuleList
        self.width_mult = max(FLAGS.width_mult_list)
        self.depth_mult = max(FLAGS.depth_mult_list)
        self.res = max(FLAGS.resolution_list)  # resolution 是从224，按照32来递减的
    ...
    def forward(self, input):  # 前向
        ...
        y = self.bn[idx + (len(FLAGS.width_mult_list) * dpx) + (len(FLAGS.resolution_list) * resid)](input)  # 找到当前的 bn
        return y
...
class USBatchNorm2d(nn.BatchNorm2d):
    def __init__(self, num_features, ratio=1):
        num_features_max = int(make_divisible(num_features * width_mult / ratio) * ratio)
        super(USBatchNorm2d, self).__init__(num_features_max, affine=True, track_running_stats=False)
        self.num_features_basic = num_features
        # for tracking log during training
        self.bn = nn.ModuleList([nn.BatchNorm2d(i, affine=False) for i in [int(make_divisible(num_features * width_mult / ratio) * ratio) for width_mult in FLAGS.width_mult_list]])
        self.ratio = ratio
        self.width_mult = None
        self.ignore_model_profiling = True
    ...
    def forward(self, input):
        weight = self.weight
        bias = self.bias
        c = int(make_divisible(self.num_features_basic * self.width_mult / self.ratio) * self.ratio)
...
class TwoInputSequential(nn.Module):  # 两个输入，如 source 和 domain。进行 sequence，

    def __init__(self, *args):
        super(TwoInputSequential, self).__init__()
        if len(args) == 1 and isinstance(args[0], nn.ModuleDict):  # 如果是 dict
            for key, module in args[0].items():
                self.add_module(key, module)
        else:  # ** 一般是用这个，加入模型中
            for idx, module in enumerate(args):
                self.add_module(str(idx), module)  # 加入模块
    ...
    def forward(self, input, dom=0):
        ...
        for module in self._modules.values():
            ...
            if str(module).startswith("DomainSpecificBatchNorm2d"):  # done ? 指定域，就是用指定的 BN
